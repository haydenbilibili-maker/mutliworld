/**
 * 默认视野、图层列表等常量，与 PRD / URL 规范一致
 */

import type { LayerId } from '@/types/geo';

/** 默认地图中心（亚太视角） */
export const DEFAULT_CENTER: [number, number] = [105, 28];
export const DEFAULT_ZOOM = 3.5;
export const DEFAULT_VIEW = 'global' as const;
export const DEFAULT_TIME_RANGE = '7d' as const;

/** 图层 ID 与中文标签 */
export const LAYER_LABELS: Record<LayerId, string> = {
  conflicts: '冲突与安全',
  conflict_zones: '冲突区',
  hotspots: '热点区域',
  bases: '基地与演习',
  economic: '经济与基建',
  waterways: '水道与港口',
  natural: '自然灾害',
  weather: '气象预警',
  live_weather: '实时天气',
  military: '军事动态',
  sanctions: '制裁与博弈',
  nuclear: '核与战略',
  outages: '基础设施',
  aviation: '航空枢纽',
  live_flights: '实时航班',
  live_fires: '实时火点',
  live_maritime: '海运实时',
  maritime: '海运要道',
  cables: '海底光缆',
  econ_hubs: '全球经济中心',
  minerals: '关键矿产',
  daynight: '昼夜分界线',
  pipelines: '油气管线',
  hydrocarbon_reserves: '油气储藏',
  datacenters: 'AI 数据中心',
  protests: '抗议活动',
  climate: '气候异常',
  launch_sites: '航天发射场',
  launch_log: '发射日志',
  semiconductors: '半导体晶圆厂',
  garrisons: '海外军事基地',
  deep_sea_mining: '深海采矿',
  tectonics: '板块与断层',
  cable_incidents: '海缆中断事件',
  quake_depth: '震源深度',
  ground_stations: '测控/地面站',
  sat_constellations: '在轨卫星（GEO）',
  space_stations: '空间站（实时）',
  satellites: '在轨卫星（实时）',
  space_debris: '空间碎片（实时）',
  space_events: '空天事件',
  marine_archaeology: '海洋考古',
  ocean_currents: '洋流',
  fisheries: '渔场',
  coral_reefs: '珊瑚礁',
  marine_life: '海洋生物',
  undersea_wonders: '海底奇观',
  migration_routes: '迁徙路线',
  monsoon: '季风气候带',
  atmospheric_circulation: '大气环流',
  deep_exploration: '深海探索',
  world_heritage: '世界遗产',
  china_heritage: '国保单位',
  megacities: '特大城市',
  dams: '水坝水电',
  ports: '主要港口',
  research_stations: '极地科考站',
  refineries: '炼油厂',
  airports: '国际机场',
  nuclear_reactors: '核电站',
  factories: '制造基地',
  financial_centers: '金融中心',
  borders: '边境口岸',
  deserts: '沙漠荒漠',
  universities: '高等学府',
  military_industry: '军工企业',
  wonder_projects: '超级工程',
  agriculture: '农业产区',
  religions: '宗教圣地',
  stadiums: '体育场馆',
  museums: '博物馆',
  islands: '主要岛屿',
  capitals: '各国首都',
  power_plants: '火电与其他',
  forests: '森林生态',
  earthquakes_historical: '历史大地震',
  tech_companies: '科技公司总部',
  media_orgs: '媒体机构',
  auto_brands: '汽车品牌总部',
  pharmaceutical: '药企生物科技',
  intl_orgs: '国际组织',
  space_companies: '商业航天',
  rare_earth: '稀土关键矿产',
  flags_of_convenience: '船旗国避税地',
  aerospace_mfg: '航空制造',
  defense_mfg: '国防军工',
  semi_supply: '半导体上游',
  chemicals: '化工与材料',
  wind_flow: '大气风场',
  ocean_flow: '洋流流场',
  air_pollutants: '化学污染物',
  particulates: '颗粒物',
  pizza_index: '披萨指数',
  wave_flow: '海浪流场',
  sea_temp: '海表温度',
  sig_wave_height: '有效波高',
  sst_anomaly: '海温异常',
  coral_baa: '珊瑚脆弱性',
  dhw: '度日热 DHW',
  air_temp: '2米气温',
  precip: '降水',
  mslp: '海平面气压',
  humidity: '相对湿度',
  cape: '对流有效位能',
  earthquakes: '实时地震',
  aurora: '极光带(实时)',
  volcanoes: '活跃火山',
  storms: '风暴气旋',
  floods: '洪水事件',
  dusthaze: '沙尘/霾',
  seaice: '海冰',
  wildfires: '野火',
  landslides: '滑坡',
  drought: '干旱',
  tempextremes: '极端气温',
  gdacs: '灾害告警(GDACS)',
  satellite_imagery: '真彩卫星云图',
  iss: '国际空间站(实时)',
  tiangong: '天宫空间站(实时)',
  persons: '人物分布',
};

/** 默认开启的图层（MVP） */
export const DEFAULT_LAYERS: LayerId[] = ['conflicts', 'economic', 'weather'];

/** 图层开关悬停说明（区分易混淆项） */
export const LAYER_HINTS: Partial<Record<LayerId, string>> = {
  conflicts: '冲突事件点标记（种子 + 缓存）',
  conflict_zones: '全球冲突区多边形填充（俄乌、中东、台海等）',
  aviation: '机场与航线枢纽（静态种子数据）',
  live_flights: 'OpenSky ADS-B 实时航迹（约 45 秒刷新）',
  live_fires: 'NASA FIRMS VIIRS 近实时活跃火点（约 10 分钟刷新，需 MAP_KEY）',
  live_maritime: '全球 AIS 船舶航迹或航运通道模拟（约 75 秒刷新）',
  weather: '区域气象预警（种子数据）',
  live_weather: 'Open-Meteo 城市实况 + RainViewer 降水雷达',
  pizza_index: '五角大楼周边披萨繁忙度（pizzint.watch OSINT）',
  cables: '海底光缆登陆点与路由线',
  maritime: '海运要道与港口节点',
  natural: '实时 USGS 地震（橙红地震波图例）≠ 油气储藏',
  hydrocarbon_reserves: '全球主要油气田/盆地（棕金储量波样式）≠ 地震',
  quake_depth: 'USGS 震源深度分带（青紫橙）· 洋底层专用 · 非油气储藏',
  persons: '地图叠加：一键铺开全部人物的驻地地理分布（与右侧「人物」面板同源；面板用于按领域/阵营筛选、看简介与关联行动、逐个定位）',
  coral_reefs: '全球代表性珊瑚礁系统（大堡礁/珊瑚大三角/红海等）· 公开海洋地理资料',
  marine_life: '标志性海洋物种聚集与保护区（鲸/鲨/海龟/磷虾等）· 示意分布',
  undersea_wonders: '海底奇观地貌（蓝洞/海沟/热液喷口/海底火山等）· 公开资料',
  migration_routes: '海洋动物洄游/迁徙通道（灰鲸/座头鲸/棱皮龟/金枪鱼等）· 示意路径',
  world_heritage: 'UNESCO 世界遗产名录（文化/自然/双重）· 全球精选真实子集',
  china_heritage: '中国全国重点文物保护单位（国保）· 精选真实子集',
  megacities: '人口 1000 万以上的特大城市与都会区节点 · 都会区人口量级',
  dams: '全球主要水坝与水电站（水电/灌溉/防洪）· 装机容量与坝高',
  ports: '全球主要海港与集装箱枢纽 · 吞吐量量级',
  research_stations: '南极与北极科考站 · 所属国与运行状态',
  refineries: '全球主要炼油厂与石化基地 · 日加工能力',
  airports: '全球主要国际机场与航空枢纽 · 客货运量级',
  nuclear_reactors: '全球在运民用核电站 · 装机容量与堆型',
  factories: '全球主要制造业与工业基地 · 汽车造船/钢铁/电子',
  financial_centers: '全球主要金融中心与交易所 · 央行/SWF/证交所',
  borders: '全球主要陆路边境口岸与关隘 · 地缘与贸易节点',
  deserts: '全球主要沙漠与荒漠 · 面积与类型',
  universities: '全球顶尖大学与高等学府 · 研究与学术重镇',
  military_industry: '全球主要军工与防务企业 · 武器/航空/航天',
  wonder_projects: '全球超级工程与基础设施奇迹 · 桥隧/摩天楼/巨型基建',
  agriculture: '全球主要农业产区与粮仓 · 粮食/经济作物主产区',
  religions: '全球主要宗教圣地与朝圣中心 · 各大宗教',
  stadiums: '全球主要体育场馆 · 足球/奥运/美式橄榄球',
  museums: '全球主要博物馆与文化机构 · 历史/艺术/科学',
  islands: '全球主要岛屿与群岛 · 面积与战略地位',
  capitals: '各国首都与行政中心 · 政治权力所在地',
  power_plants: '全球主要火电厂与其他发电设施 · 煤电/气电/风光',
  forests: '全球主要森林与生态区 · 热带/温带/北方林',
  earthquakes_historical: '历史上造成重大伤亡的大地震 · 震级与影响',
  tech_companies: '全球主要科技公司总部 · 互联网/软件/云/AI',
  media_orgs: '全球主要媒体与新闻机构总部 · 通讯社/电视/报纸',
  auto_brands: '全球主要汽车品牌总部 · 整车制造商',
  pharmaceutical: '全球主要药企与生物科技公司 · 疫苗/创新药',
  intl_orgs: '全球主要国际组织与多边机构总部 · 联合国/世行/WTO/IEA',
  space_companies: '全球商业航天与卫星公司总部 · 火箭/卫星/星座',
  rare_earth: '全球稀土与关键矿产供应链节点 · 开采/加工/精炼',
  flags_of_convenience: '全球主要船旗国与避税地 · 海运注册/离岸金融',
  aerospace_mfg: '全球航空制造产业链 · 飞机总装厂/发动机/Tier1 供应商',
  defense_mfg: '全球国防军工承包商 · 主承包商/造船厂/导弹',
  semi_supply: '全球半导体上游供应链 · EDA/封测/硅晶圆/材料（与晶圆厂图层互补）',
  chemicals: '全球化工与材料产业链 · 石化/特种化学/化肥/电池材料',
  wind_flow: '近地大气风场粒子流（真实 GFS/Open-Meteo 10m 风）· 对标 earth.nullschool',
  ocean_flow: '海洋表层洋流流场（粒子动画）· 后续轮次接入',
  air_pollutants: '化学污染物标量叠加（CO/SO₂/NO₂ 等）· 后续轮次接入',
  particulates: '颗粒物浓度叠加（PM2.5/PM10）· 后续轮次接入',
};
