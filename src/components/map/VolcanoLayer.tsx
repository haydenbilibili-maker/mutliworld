...
/**
 * 活跃火山图层 — 地表层（NASA EONET，近实时）。橙红圆点 + 点击弹出名称/起始日期。真实数据·中立并陈。
 */
...
const SOURCE = 'live-volcanoes';
const GLOW = 'live-volcanoes-glow';
const CORE = 'live-volcanoes-core';
const POPUP_BG = '#0A0E17';
const fetcher = (url: string) => fetch(url).then((r) => r.json());

type VolcProps = { title: string; date: string };
...
export function VolcanoLayer() {
  ...
  const enabled = activeTier === 'surface' && activeLayers.includes('volcanoes');
  const { data } = useSWR<FeatureCollection>(enabled ? '/api/volcanoes' : null, fetcher, {
      ...
      try {
        ...
        if (!map.getLayer(GLOW)) {
          map.addLayer({
            ...
            paint: { 'circle-radius': 11, 'circle-color': '#f97316', 'circle-opacity': 0.25, 'circle-blur': 0.8 },
          }, findLiveOverlayBeforeId(map));
        }
        if (!map.getLayer(CORE)) {
          map.addLayer({
            id: CORE, type: 'circle', source: SOURCE, layout: { visibility: 'none' },
            paint: { 'circle-radius': 5, 'circle-color': '#ea580c', 'circle-opacity': 0.95, 'circle-stroke-width': 1, 'circle-stroke-color': '#fed7aa' },
          }, findLiveOverlayBeforeId(map));
        }
      } catch { /* 样式未就绪 */ }
    };
  ...
  useEffect(() => {
    ...
    const onClick = (e: maplibregl.MapMouseEvent & { features?: maplibregl.MapGeoJSONFeature[] }) => {
      ...
      const p = f.properties as unknown as VolcProps;
      ...
      const popup = new maplibregl.Popup({ closeButton: true, closeOnClick: true, offset: 10, className: 'geodata-popup' })
        .setLngLat(coords)
        .setHTML(`<div style="font-size:13px;line-height:1.5;min-width:11rem"><div style="font-weight:600;margin-bottom:4px">🌋 ${p.title}</div><div style="font-size:11px;color:#94a3b8">活跃事件 ${date}</div></div>`)
        .addTo(map);
      applyPopupTheme(popup);
      popupRef.current = popup;
      const detail: EventDetail = {
        id: `volcano-${coords[0]},${coords[1]}`,
        title: `🌋 ${p.title}`,
