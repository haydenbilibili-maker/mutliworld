...
/**
 * 风暴/热带气旋图层 — 地表层（NASA EONET，近实时）。青白气旋圆点 + 点击弹出名称/最新时刻。真实数据·中立并陈。
 */
...
const SOURCE = 'live-storms';
const GLOW = 'live-storms-glow';
const CORE = 'live-storms-core';
const POPUP_BG = '#0A0E17';
const fetcher = (url: string) => fetch(url).then((r) => r.json());

type StormProps = { title: string; date: string };
...
export function StormLayer() {
  ...
  const enabled = activeTier === 'surface' && activeLayers.includes('storms');
  const { data } = useSWR<FeatureCollection>(enabled ? '/api/storms' : null, fetcher, {
      ...
      try {
        ...
        if (!map.getLayer(GLOW)) {
          map.addLayer({
            ...
            paint: { 'circle-radius': 13, 'circle-color': '#38bdf8', 'circle-opacity': 0.22, 'circle-blur': 0.9 },
          }, findLiveOverlayBeforeId(map));
        }
        if (!map.getLayer(CORE)) {
          map.addLayer({
            id: CORE, type: 'circle', source: SOURCE, layout: { visibility: 'none' },
            paint: { 'circle-radius': 5, 'circle-color': '#7dd3fc', 'circle-opacity': 0.95, 'circle-stroke-width': 1, 'circle-stroke-color': '#e0f2fe' },
          }, findLiveOverlayBeforeId(map));
        }
      } catch { /* 样式未就绪 */ }
    };
  ...
  useEffect(() => {
    ...
    const onClick = (e: maplibregl.MapMouseEvent & { features?: maplibregl.MapGeoJSONFeature[] }) => {
      ...
      const p = f.properties as unknown as StormProps;
