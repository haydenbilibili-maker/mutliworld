...
/** 风暴/热带气旋图层数据 — 地表层（NASA EONET severeStorms，免密钥真实，近实时）。服务端缓存 30 分钟。 */
export async function GET() {
  try {
    const body = await getEonetGeoJSON('severeStorms', 'NASA EONET 自然事件追踪器（风暴）');
    return NextResponse.json(body, { headers: { 'Cache-Control': 'public, s-maxage=1800, stale-while-revalidate=3600' } });
  } catch (err) {
    const stale = getEonetStale('severeStorms');
