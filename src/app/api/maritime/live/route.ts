 ...
 *
 * 全球海运 AIS 代理（AISStream 或航运通道模拟）。
 *
 * Query:
 *   lamin,lomin,lamax,lomax — 可选 bbox（zoom ≥ 4 时传入）
 *   limit — 返回上限（默认 global 600 / bbox 1200，最大 1200）
 */
export async function GET(req: NextRequest) {
  ...
  try {
    const { vessels, source, sourceLabel, total, capped, simulated, error } =
      await fetchMaritimeVessels({ bbox, limit });
    ...
    const message = err instanceof Error ? err.message : '获取海运数据失败';
