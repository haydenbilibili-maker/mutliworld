...
/**
 * 洪水事件图层数据 — 地表层（NASA EONET 自然事件追踪器）。
 * 真实数据：NASA EONET v3（category=floods, status=open），免费无 key，近实时。
 * 取每个事件最新位置，转 GeoJSON 点（名称/最新时刻）。服务端缓存 30 分钟。
 */

const FEED = 'https://eonet.gsfc.nasa.gov/api/v3/events?category=floods&status=open';
...
let cache: { expires: number; body: FloodFC } | null = null;
    ...
    const features = events
      .map((ev) => {
        ...
        return {
          ...
          properties: { title: String(ev.title ?? '洪水事件'), date: String(last.date ?? '') },
        };
      })
      .filter((f): f is NonNullable<typeof f> => f !== null);
    const body: FloodFC = { type: 'FeatureCollection', features, generatedAt: new Date().toISOString(), source: 'NASA EONET 自然事件追踪器（洪水）' };
