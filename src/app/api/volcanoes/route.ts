...
/**
 * 活跃火山图层数据 — 地表层（NASA EONET 自然事件追踪器）。
 * 真实数据：NASA EONET v3（category=volcanoes, status=open），免费无 key，近实时。
 * 取每个事件最新位置，转 GeoJSON 点（名称/起始日期）。服务端缓存 30 分钟。
 */

const FEED = 'https://eonet.gsfc.nasa.gov/api/v3/events?category=volcanoes&status=open';
...
let cache: { expires: number; body: VolcanoFC } | null = null;
    ...
    const features = events
      .map((ev) => {
        ...
        return {
          ...
          properties: { title: String(ev.title ?? '活跃火山'), date: String(last.date ?? '') },
        };
      })
      .filter((f): f is NonNullable<typeof f> => f !== null);
    const body: VolcanoFC = { type: 'FeatureCollection', features, generatedAt: new Date().toISOString(), source: 'NASA EONET 自然事件追踪器' };
